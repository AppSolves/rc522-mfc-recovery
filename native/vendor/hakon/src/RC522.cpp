...
RC522::RC522() : currentlyAuthenticated(0xFF), quiet_(true), UID(0)
{
#if DBG
...
bool RC522::selectCard(unsigned attempts, unsigned delayMs)
{
    for (unsigned attempt = 0; attempt < attempts; ++attempt)
    {
        ...
        delay(delayMs);
#endif
        if (sendReqA() && anticollision() && select())
        {
            return true;
        }
    }
...
bool RC522::authenticateOnChip( byte command, byte blockAddr, byte *key /*=nullptr*/ )
{
	...
	if( key != nullptr )
	{
	    for( int i = 0; i<6; i++ )// store the complete validated key
	    {
    ...
    if (!quiet_) std::cerr << "Could not authenticate\n";
    return false;
...
bool RC522::readBlock( byte blockAddr, byte *data, byte len )
{
    ...
    if( len < 18 )
    {
	if (!quiet_) std::cerr << "Could not read: No storage\n";
	return false;
    }

    if( blockAddr > 63 )
    {
	if (!quiet_) std::cerr << "Could not read: Invalid address\n";
	return false;
    }

    if( currentlyAuthenticated != blockAddr/4 )// sector not already authenticated
    {
        if( !authenticateOnChip( AUTHENT_A, blockAddr) )
        {
            return false;
        }
    }
...
void RC522::setQuiet(bool quiet)
{
    quiet_ = quiet;
}
...
bool RC522::halt()
{
    ...
    if (!quiet_) std::cerr << "Could not halt\n";
    return false;
}
...
void RC522::writeRegister( byte reg, byte *val, const byte len /*=1*/ )
{
    ...
    [[maybe_unused]] byte data[ len + 1];// addr + val
    // select register
...
bool RC522::piccIO( byte command, byte nrOfBytesToSend, byte *data, byte len, byte nrOfLastBits /*=0*/ )
{
    ...
    {
	if (!quiet_) std::cerr << "Error occurred\n";
	return false;
    }
	...
	if( status > len )
	{
	    if (!quiet_) std::cerr << "Not enough room in buffer\n";
	    return false;
	}
